    ...
    return Scaffold(
          ...
          actions: [
            Obx(() => IconButton(
                onPressed: () async {
                  ...
                  isSignedIn.value
                      ? servicesCart.isNotEmpty
                          ...
                          : Get.snackbar('Warning', 'Please select some services before', backgroundColor: kWhiteColor)
                      : Get.to(() => LoginView());
                ...
                icon: Badge(
                    ...
                    badgeContent: Text(servicesCart.length.toString(), style: kWhiteText),
                    child: Icon(Glyphicon.cart3, color: servicesCart.isEmpty ? kGreyColor : kWhiteColor))))
          ],
        ),
        body: SingleChildScrollView(child: Obx(() {
          if (salonsCntrlr.salons == null) {
            ...
            SalonModel salon = salonsCntrlr.salons!.firstWhere((salon) => salonId == salon.uid);
                  ...
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      RatingBarIndicator(
                          ...
                          itemBuilder: (context, rat) => const Icon(Glyphicon.star_fill, color: Color(0xFFFFCA28))),
                      const SizedBox(width: 4),
                      Text(salon.rating.toString(), style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                    ]),
                    const Divider(),
                    GestureDetector(
                      onTap: () {
                        ...
                        ? salon.services!.isNotEmpty
                            ? ListView.separated(
                                ...
                                itemBuilder: (context, index) {
                                  ServicesModel service = salon.services![index];
                                  ...
                                  return ListTile(
                                      ...
                                      leading: SizedBox(
                                        ...
                                        child: ClipRRect(
                                          ...
                                          child: NetImage(imagePath: service.image ?? ''),
                                        ),
                                      ),
                                      title: Text(service.name!, style: kBoldText),
                                      subtitle: Text('Rs: ' + service.price.toString()),
                                      trailing: Obx(() => Checkbox(
                                          checkColor: kWhiteColor,
                                          activeColor: kMainColor,
                                          value: servicesCart.contains(service),
                                          onChanged: (value) {
                                            servicesCart.contains(service)
                                                ? servicesCart.remove(service)
                                                : servicesCart.add(service);
                                            double total = 0.0;
